...
def __get_juce_library_path(root_path: str) -> str:
    ...
    for target in __target_paths:
        ...
        if target: current_base_path = __os.path.join(current_base_path, target)

        library_path = __os.path.abspath(__os.path.join(current_base_path, __get_juce_library()))
        if __os.path.exists(library_path):
            return library_path
...
def __get_juce_defs_path(root_path: str) -> str:
    ...
    for target in __target_paths:
        ...
        defs_path = __os.path.abspath(__os.path.join(current_base_path, "Defs.txt"))
        if __os.path.exists(defs_path):
            return defs_path
...
def juce_bootstrap():
    ...
    __cppyy.add_include_path(include_path)
    __cppyy.load_library(library_path)
    __cppyy.cppdef(defines)
